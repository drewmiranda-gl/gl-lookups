...
if [ -z "$1" ]
then
      echo "ERROR! you must specify 4 arguments:"
      echo "service-wrapper.sh <hostname> <port> <logfile> <healthcheck-file>"
      exit 1
else
      bindhostname=$1
fi
...
$get_latest_python_full_path /opt/graylog/lookup-service/web.py --hostname $bindhostname --port $portnum --log $logfile --healthcheck-file $healthcheckfile --sql-host 192.168.0.1 --sql-user root --sql-pass $(cat /opt/graylog/lookup-service/sql.secret)
